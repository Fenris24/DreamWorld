...
public class DarkDimension extends MasterMap
{
    Tile[] Layer;
    public DarkDimension(MapParser.MapHandler handle)
    {
        super(handle);
        Layer = AssetManager.getInstance().getTiles("Dark Dimension");
    }
        ...
        for(int worldRow = 0; worldRow < mapHeight; worldRow++)
        {
            for(int worldCol = 0; worldCol < mapWidth; worldCol++)
            {
                ...
                if (insideWindow(worldX, worldY, entityX, entityY))
                {
                    ...
                    DrawLayer(g2d, Layer, mapMatrix[2], 0, worldRow, worldCol, screenX, screenY);
                    /*
